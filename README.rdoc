...
Red Hat, CentOS, Fedora, ArchLinux, and Amazon Linux can be used but will be converted to a Debian/Ubuntu style Apache as it's far easier to manage with Chef.
...
Both forward and reverse proxies are supported. You can configure either or both at the same time if desired.

To enable a forward proxy, set +[:apache][:proxy][:forward]+ to true. The default is false.

To enable a reverse proxy, set +[:apache][:proxy][:reverse]+ to an array of hashes. Each hash contains the servername for the virtual host and an array of URL maps.

    {
      "servername": "dev.hrworx.com",
      "url_map": [ ... ]
    }

+url_map+ is itself an array of hashes, each listing the URL to proxy (i.e. what the browser sends) and the URL to forward it to.

    {
      "from": "/external/endpoint",
      "to": "http://internal.localdomain/internal/endpoint"
    }

Both +ProxyPass+ and +ProxyPassReverse+ are automatically configured.
